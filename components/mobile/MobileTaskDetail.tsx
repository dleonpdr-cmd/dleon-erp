...
const EVENT_LABEL: Record<string, string> = {
  ...
  inspection_approved: 'Inspeção aprovada', inspection_rework: 'Repasse solicitado',
}
...
export default function MobileTaskDetail({ ctx, task, events }: Props) {
  ...
  const isMyTask = ['repair', 'rework'].includes(task.step_type) ||
    (ctx.activeRole === 'inspector' && task.step_type === 'inspection') ||
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#0D0D0D', paddingBottom: '24px' }}>
      ...
      {/* Timer (se em andamento) */}
      {task.status === 'in_progress' && (
        <div style={{ background: '#FF6B0011', borderBottom: '1px solid #FF6B0033', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '10px', color: '#FF6B0088', letterSpacing: '0.08em', fontWeight: '600' }}>EM ANDAMENTO</div>
            <div style={{ fontSize: '11px', color: '#FF6B00AA', marginTop: '2px' }}>{STEP_LABEL[task.step_type] ?? task.step_type}{task.round > 1 ? ` · Round ${task.round}` : ''}</div>
          </div>
          <div style={{ fontSize: '28px', fontWeight: '800', color: '#FF6B00', fontVariantNumeric: 'tabular-nums' }}>{timer}</div>
        </div>
      )}
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ background: '#141414', border: '1px solid #1E1E1E', borderRadius: '12px', padding: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '20px', fontWeight: '700', color: '#FF6B00' }}>#{task.queue_position}</div>
          </div>
        </div>
          ...
          <div style={{ marginTop: '24px' }}>
            ...
            {task.status === 'queued' && (
              <button
                onClick={handleStart}
                disabled={pending}
                style={{ width: '100%', height: '56px', background: '#FF6B00', border: 'none', borderRadius: '14px', color: '#fff', fontSize: '16px', fontWeight: '700', cursor: 'pointer', opacity: pending ? 0.6 : 1 }}
              >
        ...
        {events.length > 0 && (
          <>
            <div style={{ fontSize: '10px', color: '#444', letterSpacing: '0.08em', fontWeight: '600', margin: '24px 0 10px' }}>HISTÓRICO</div>
